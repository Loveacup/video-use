...
Read install.md first to install this repo, wire up ffmpeg, register the skill with whichever agent you're running under, and prefetch the on-device transcription models. Then read SKILL.md for daily usage, and always read helpers/ because that's where the editing scripts live. After install, don't transcribe anything on your own — just tell me it's ready and wait for me to drop footage into a folder.
```

The agent handles the clone, dependencies, skill registration, and model download. Transcription runs on-device (Qwen3-ASR + Qwen3-ForcedAligner on MLX, Apple Silicon) — no API key, no audio leaves your machine.
...
**Layer 1 — Audio transcript (always loaded).** One on-device pass per source (Qwen3-ASR, then Qwen3-ForcedAligner) gives word-level timestamps and verbatim fillers; `--num-speakers` adds pyannote diarization. All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view.
